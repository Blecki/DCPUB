...
namespace DCPUB.Emulator
{
    ...
    public class Emulator
    {
        ...
        public String Disassemble(ref ushort PC)
        {
            ...
            var str = Hex.hex(PC) + " " + ((Assembly.Instructions)iid).ToString() + " ";
             PC += 1;
        ...
        public void Step()
        {
            ...
            var ins = (DCPUB.Assembly.Instructions)iid;
            ...
            if (SkipIf == false) 
            {
                if (ins < Assembly.Instructions.SINGLE_OPERAND_INSTRUCTIONS)
                    GetOperandValue(operandB, OperandPlace.B, true);
                if (ins < Assembly.Instructions.IFB || ins > Assembly.Instructions.IFU)
                    SkipIf = true;
                return;
            }
            ...
            switch (ins)
            {
                case Assembly.Instructions.SET: //0x01 | SET b, a | sets b to a
                    AssignToOperand(operandB, value, false);
                    break;
                case Assembly.Instructions.ADD: //ADD b, a | sets b to b+a, sets EX to 0x0001 if there's an overflow, 0x0 otherwise
                    ...
                    break;
                case Assembly.Instructions.SUB: //SUB b, a | sets b to b-a, sets EX to 0xffff if there's an underflow, 0x0 otherwise
                    intermediate = (UInt32)GetOperandValue(operandB, OperandPlace.B) - value;
                    AssignToOperand(operandB, (ushort)intermediate, true);
                    AssignToEx(intermediate >> 16);
                    break;
                case Assembly.Instructions.MUL: //MUL b, a | sets b to b*a, sets EX to ((b*a)>>16)&0xffff (treats b, a as unsigned)
                    intermediate = (UInt32)GetOperandValue(operandB, OperandPlace.B) * value;
                    AssignToOperand(operandB, (ushort)intermediate, true);
                    AssignToEx(intermediate >> 16);
                    break;
                case Assembly.Instructions.MLI: //MLI b, a | like MUL, but treat b, a as signed
                ...
                case Assembly.Instructions.AND: //AND b, a | sets b to b&a
                    ...
                    break;
                case Assembly.Instructions.BOR: //BOR b, a | sets b to b|a
                    AssignToOperand(operandB, (ushort)(GetOperandValue(operandB, OperandPlace.B) | value), true);
                    break;
                case Assembly.Instructions.XOR: //XOR b, a | sets b to b^a
                    AssignToOperand(operandB, (ushort)(GetOperandValue(operandB, OperandPlace.B) ^ value));
                    break;
                case Assembly.Instructions.SHR: //SHR b, a | sets b to b>>>a, sets EX to ((b<<16)>>a)&0xffff (logical shift)
                    ...
                    break;
                case Assembly.Instructions.ASR: //ASR b, a | sets b to b>>a, sets EX to ((b<<16)>>>a)&0xffff (arithmetic shift) (treats b as signed)
                    intermediate = (UInt32)GetOperandValue(operandB, OperandPlace.B);
                    AssignToOperand(operandB, (ushort)((int)intermediate >> value), true);
                    AssignToEx((ushort)((int)intermediate << 16 >> value));
                    break;
                case Assembly.Instructions.SHL: //SHL b, a | sets b to b<<a, sets EX to ((b<<a)>>16)&0xffff
                    intermediate = (UInt32)GetOperandValue(operandB, OperandPlace.B);
                    AssignToOperand(operandB, (ushort)(intermediate << value), true);
                    AssignToEx((ushort)(intermediate << value >> 16));
                    break;
                case Assembly.Instructions.IFB: //IFB b, a | performs next instruction only if (b&a)!=0
                    SkipIf = (GetOperandValue(operandB, OperandPlace.B) & value) != 0;
                    break;
                case Assembly.Instructions.IFC: //IFC b, a | performs next instruction only if (b&a)==0
                    SkipIf = (GetOperandValue(operandB, OperandPlace.B) & value) == 0;
                    break;
                case Assembly.Instructions.IFE: //IFE b, a | performs next instruction only if b==a 
                    SkipIf = GetOperandValue(operandB, OperandPlace.B) == value;
                    break;
                case Assembly.Instructions.IFN: //IFB b, a | performs next instruction only if b!=a 
                    SkipIf = GetOperandValue(operandB, OperandPlace.B) != value;
                    break;
                case Assembly.Instructions.IFG: //IFG b, a | performs next instruction only if b>a 
                    SkipIf = GetOperandValue(operandB, OperandPlace.B) > value;
                    break;
                case Assembly.Instructions.IFA: //IFA b, a | performs next instruction only if b>a (signed)
                    SkipIf = (int)GetOperandValue(operandB, OperandPlace.B) > (int)value;
                    break;
                case Assembly.Instructions.IFL: //IFL b, a | performs next instruction only if b<a 
                    SkipIf = GetOperandValue(operandB, OperandPlace.B) < value;
                    break;
                case Assembly.Instructions.IFU: //IFU b, a | performs next instruction only if b<a (signed)
                    SkipIf = (int)GetOperandValue(operandB, OperandPlace.B) < (int)value;
                    break;
                case Assembly.Instructions.ADX: //ADX b, a | sets b to b+a+EX, sets EX to 0x0001 if there is an over-flow, 0x0 otherwise
                    intermediate = (UInt32)GetOperandValue(operandB, OperandPlace.B) + value + registers[(int)Registers.EX];
                    AssignToOperand(operandB, (ushort)intermediate, true);
                    AssignToEx(intermediate >> 16);
                    break;
                case Assembly.Instructions.SBX: //SBX b, a | sets b to b-a+EX, sets EX to 0xFFFF if there is an under-flow, 0x0 otherwise
                    intermediate = (UInt32)GetOperandValue(operandB, OperandPlace.B) - value + registers[(int)Registers.EX];
                    AssignToOperand(operandB, (ushort)intermediate, true);
                    AssignToEx(intermediate >> 16);
                    break;
                case Assembly.Instructions.STI: //STI b, a | sets b to a, then increases I and J by 1
                    AssignToOperand(operandB, value, false);
                    registers[(int)Registers.I] += 1;
                    registers[(int)Registers.J] += 1;
                    break;
                case Assembly.Instructions.STD: //STI b, a | sets b to a, then decreases I and J by 1
                    AssignToOperand(operandB, value, false);
                    registers[(int)Registers.I] -= 1;
                    registers[(int)Registers.J] -= 1;
                    break;

                case Assembly.Instructions.JSR: //pushes the address of the next instruction to the stack, then sets PC to a
                    ram[(ushort)(registers[(int)Registers.SP] - 1)] = registers[(int)Registers.PC];
                    registers[(int)Registers.SP] -= 1;
                    registers[(int)Registers.PC] = value;
                    break;
                case Assembly.Instructions.HLT:
                    throw new Exception("HALT " + Hex.hex(value));

                case Assembly.Instructions.INT: //INT a | triggers a software interrupt with message a
                    TriggerInterrupt(value);
                    break;
                case Assembly.Instructions.IAG: //IAG a | sets a to IA 
                    AssignToOperand(operandA, registers[(int)Registers.IA], true);
                    break;
                case Assembly.Instructions.IAS: //IAS a | sets IA to A 
                    registers[(int)Registers.IA] = value;
                    break;
                case Assembly.Instructions.RFI: //RFI a | disables interrupt queueing, pops A from the stack, then pops PC from the stack
                    protectInterruptQueue.WaitOne();
                    ...
                    registers[(int)Registers.SP] += 2;
                    break;
                case Assembly.Instructions.IAQ: //IAQ a | if a is nonzero, interrupts will be added to the queue
                    interruptQueueEnabled = value != 0; //instead of triggered. if a is zero, interrupts will be
                    break;                              //triggered as normal again

                case Assembly.Instructions.HWN: //HWN a | sets a to number of connected hardware devices
                    AssignToOperand(operandA, (ushort)devices.Count, true);
                    break;
                case Assembly.Instructions.HWQ: //HWQ a | sets A, B, C, X, Y registers to information about hardware a 
                    ...
                    break;
                case Assembly.Instructions.HWI: //HWI a | sends an interrupt to hardware a
